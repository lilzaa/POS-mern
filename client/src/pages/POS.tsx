...
import { Search, Minus, Plus, Trash2, CreditCard, Receipt, X, Check } from "lucide-react";
...
function POS() {
  ...
  const [receipt, setReceipt] = useState<{ items: CartItem[]; total: number; invoice: string } | null>(null);
  ...
  const update = (id: string, d: number) =>
    setCart((c) => c.flatMap((x) => (x.product.id === id ? (x.qty + d <= 0 ? [] : [{ ...x, qty: x.qty + d }]) : [x])));
  const remove = (id: string) => setCart((c) => c.filter((x) => x.product.id !== id));

  const checkout = () => {
    if (!cart.length) return;
    setReceipt({ items: cart, total, invoice: `INV-${Math.floor(10000 + Math.random() * 89999)}` });
    setCart([]);
  };

  return (
    <PageShell title="POS Billing" subtitle="The fastest way to ring up a sale.">
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-5">
        ...
        <div className="glass rounded-3xl p-5">
          <div className="flex flex-col md:flex-row gap-3 mb-4">
            <div className="relative flex-1">
              ...
              <input
                value={q} onChange={(e) => setQ(e.target.value)} placeholder="Scan or search product…"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
            {items.map((p) => (
              <motion.button
                ...
                onClick={() => add(p)} disabled={p.stock === 0}
              ...
              >
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition" style={{ background: "linear-gradient(135deg, oklch(0.72 0.16 162 / 0.12), transparent)" }} />
                <div className="relative">
                  <div className="grid h-14 w-14 place-items-center rounded-xl bg-background text-2xl mb-3">{p.image}</div>
                  <div className="text-sm font-semibold leading-tight">{p.name}</div>
                  <div className="mt-0.5 text-[11px] text-muted-foreground">{p.category}</div>
                  <div className="mt-3 flex items-baseline justify-between">
                    <div className="font-bold gold-text">${p.price.toFixed(2)}</div>
                    <div className="text-[10px] text-muted-foreground">{p.stock === 0 ? "Out of stock" : `${p.stock} in stock`}</div>
                  </div>
                </div>
              </motion.button>
            ))}
        ...
        <div className="glass rounded-3xl p-5 flex flex-col max-h-[80vh]">
          ...
          <div className="mt-4 flex-1 overflow-y-auto -mx-2 px-2 space-y-2 min-h-[200px]">
            <AnimatePresence initial={false}>
              ...
              {cart.map((c) => (
                ...
                >
                  <div className="grid h-11 w-11 place-items-center rounded-xl bg-background text-xl">{c.product.image}</div>
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium">{c.product.name}</div>
                  ...
                  <div className="flex items-center gap-1 rounded-lg border border-border bg-background/50">
                    <button onClick={() => update(c.product.id, -1)} className="grid h-7 w-7 place-items-center hover:text-primary"><Minus className="h-3 w-3" /></button>
                    <span className="w-6 text-center text-sm font-semibold">{c.qty}</span>
                    <button onClick={() => update(c.product.id, +1)} className="grid h-7 w-7 place-items-center hover:text-primary"><Plus className="h-3 w-3" /></button>
                  </div>
                  <button onClick={() => remove(c.product.id)} className="text-muted-foreground hover:text-destructive">
                    <X className="h-4 w-4" />
                  </button>
                </motion.div>
              ))}
          ...
          <motion.button
            whileHover={{ scale: cart.length ? 1.01 : 1 }} whileTap={{ scale: 0.99 }}
            ...
            >
              <div className="flex flex-col items-center text-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.1, type: "spring" }}
                  className="grid h-14 w-14 place-items-center rounded-full shadow-glow" style={{ background: "var(--gradient-primary)" }}>
                  <Check className="h-7 w-7 text-primary-foreground" />
                </motion.div>
                <h3 className="mt-4 text-2xl font-bold">Payment successful</h3>
                <p className="text-sm text-muted-foreground">Invoice {receipt.invoice}</p>
              </div>
              ...
              <div className="mt-4 flex justify-between text-lg font-bold">
                ...
                <span className="gradient-text">${receipt.total.toFixed(2)}</span>
              </div>
              <button onClick={() => setReceipt(null)} className="mt-6 w-full rounded-xl py-3 font-semibold border border-border hover:bg-secondary">
                New order
              </button>
            </motion.div>
          </motion.div>
...
function Row({ label, value, large }: { label: string; value: number; large?: boolean }) {
  return (
    <div className={`flex justify-between ${large ? "text-lg font-bold pt-2" : "text-muted-foreground"}`}>
      <span>{label}</span>
      <span className={large ? "gradient-text" : "text-foreground font-medium"}>${value.toFixed(2)}</span>
    </div>
  );
}
